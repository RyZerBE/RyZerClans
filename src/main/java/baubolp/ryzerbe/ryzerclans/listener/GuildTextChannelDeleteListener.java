...
public class GuildTextChannelDeleteListener extends ListenerAdapter {
    ...
    public void onTextChannelDelete(@Nonnull TextChannelDeleteEvent event) {
        ...
        if (RyZerClans.getDatabaseManager().validClanDiscord(guildId)) {
            if (!event.getChannel().getId().equals(RyZerClans.getDatabaseManager().getClanWarInformationChannel(guildId))) return;
                ...
                for (TextChannel textChannel : event.getGuild().getTextChannels()) {
                    ...
                    RyZerClans.getDatabaseManager().resetClanWarChannel(event.getGuild().getId());
                }
        }
    }
}
